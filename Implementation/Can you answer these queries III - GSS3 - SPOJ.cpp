...
class SegTree
{
    ...
    public:
        ...
        void updateSegTree(int treeIndex,int lo,int hi,int arrIndex,int val)
        {
            if(lo==hi)
            {
                tree[treeIndex].l=tree[treeIndex].r=tree[treeIndex].s=tree[treeIndex].b=val;
                return;
            }
            int mid = lo + (hi-lo)/2;
            if(arrIndex>mid) updateSegTree(2*treeIndex+1,mid+1,hi,arrIndex,val);
            else if(arrIndex<=mid) updateSegTree(2*treeIndex,lo,mid,arrIndex,val);
